...
def scrape_undp_tenders():
    ...
    for tender in tenders:
        title_label = tender.find('div', class_='vacanciesTable__cell__label', text=lambda x: x and 'Title' in x.strip())
        ...
        ref_no_label = tender.find('div', class_='vacanciesTable__cell__label', text=lambda x: x and 'Ref No' in x.strip())
        reference_number = ref_no_label.find_next_sibling('span').text.strip() if ref_no_label else "N/A"

        # Find the deadline
        deadline_label = tender.find('div', class_='vacanciesTable__cell__label', text=lambda x: x and 'Deadline' in x.strip())
        ...
        # Log the deadline string found
        print(f"Deadline string found for tender '{title}': {deadline_str}")

        # Extract just the date part
        try:
            # Use regex to find the date in the format dd-MMM-yy
        ...
        try:
            insert_tender_to_db(tender_data)
